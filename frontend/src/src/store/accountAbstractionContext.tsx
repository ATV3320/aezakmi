...
const AccountAbstractionProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const myabi=[
   ...
   let iface = new ethers.Interface(myabi);
  ...
  const relayTransaction = async () => {
    if (web3Provider) {
      ...
      const dumpSafeTransafer: MetaTransactionData[] = [
        {
          to: "0x5696145E608cF4B3d43B11f5aA3F285fbfE9acBa",
          data: iface.encodeFunctionData("data",),
  ...
  const fetchErc20SafeBalances = useCallback(async (): Promise<Record<string, ERC20Token>> => {
    ...
    return Promise.all(
      chain.supportedErc20Tokens?.map((erc20Address:any) =>
